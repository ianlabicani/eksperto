...
class ContactController extends Controller
{
    public function update(Request $request)
    {
        ...
        if (!$request->has('contacts')) {
            return redirect()->route('expert.profile.index')->with('warning', 'Please provide at least one contact information.');
        }
        ...
        if ($user->contacts()->count() === 0 && count($validated['contacts']) === 0) {
            return redirect()->route('expert.profile.index')->with('status', 'contact-updated');
        }
